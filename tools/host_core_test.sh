...
CORE_SOURCES=("$CORE/src/action_dispatcher.cpp" "$CORE/src/arbiter.cpp" "$CORE/src/logging.cpp"
  "$CORE/src/referee_protocol.cpp" "$CORE/src/replay.cpp" "$CORE/src/world_model.cpp")
SIM_SOURCES=("$SIM/src/referee_simulator.cpp" "$SIM/src/nav_simulator.cpp"
  "$SIM/src/decision_actuator_sim.cpp")

for test in test_arbiter test_logging test_world_model test_referee_protocol test_replay test_config test_action_dispatcher; do
